...
(rpcEnvWorker: RpcEnv, val driverUrl: String, val execId: Int) {
  ...
  class ProxyEndpoint(override val rpcEnv: RpcEnv,
    ...
    override def receive: PartialFunction[Any, Unit] = {
      ...
      case LaunchTask(taskId, data) =>
        if (taskLaunched == totalTask && taskFailed == 0) {
          ...
          driver.get.send(StatusUpdate(execId.toString, taskId,
            TaskState.KILLED, data, 0, Map.empty))
        } else {
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      case RegisterExecutor(executorId, executorRef, hostPort, cores,
      logUrls, attributes, resources, resourceProfileId) =>
        logInfo("Connecting to driver: " + driverUrl)
        rpcEnv.asyncSetupEndpointRefByURI(driverUrl).flatMap { ref =>
          // This is a very fast action so we can use "ThreadUtils.sameThread"
          driver = Some(ref)
          logInfo(ref.address.toString)
          ref.ask[Boolean](RegisterExecutor(executorId, self, hostPort, cores,
            logUrls, attributes, resources, resourceProfileId))
